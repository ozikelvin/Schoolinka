...
import { PostI , PostPaginationI, UpdatePostI } from "../Interfaces/blog.interface";
...
export class PostService {
    ...
    getAllBlogPost = async (blogDataP: PostPaginationI) => {
        ...
        const queryBuilder = this.dataSource.getRepository(Post).createQueryBuilder('post');
    ...
    deleteBlog = async(blogId: string) =>{
        ...
        if(!blog) throw "Invalid Blog id";
        const deleteABlog = await this.dataSource.getRepository(Post).delete(blog.id)
    }

    searchBlog = async(query:string) =>{
        const results = await this.dataSource.getRepository(Post)
        .createQueryBuilder("post")
        .where('post.title LIKE :keywords OR post.body LIKE :keywords', { keywords: `%${query}%` })
        .getMany();
        return results;
    }
}